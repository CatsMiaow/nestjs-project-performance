...
export const configuration = async (): Promise<Config> => {
  const { config } = <{ config: Default }> await import(`${__dirname}/default`);
  const { config: environment } = <{ config: Production }> await import(`${__dirname}/${process.env.NODE_ENV || 'development'}`);
